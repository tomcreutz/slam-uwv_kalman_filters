...
FilterState processModel(
    ...
    const SimplePoseUKF::PoseUKFParameter &filter_parameter,
    double delta_time) {
  ...
  projection->navToWorld(state.position.x(), state.position.y(), latitude,
                         longitude);
  Eigen::Vector3d earth_rotation =
      Eigen::Vector3d(pose_estimation::EARTHW * cos(latitude), 0.,
  ...
  Eigen::Vector3d gyro_bias_delta =
      (-1.0 / filter_parameter.gyro_bias_tau) *
  ...
  Eigen::Vector3d acc_bias_delta =
      (-1.0 / filter_parameter.acc_bias_tau) *
  ...
  water_density_delta << (-1.0 / filter_parameter.water_density_tau) *
                             (state.water_density(0) - water_density_offset);
...
// measurement models
template <typename FilterState>
Eigen::Matrix<TranslationType::scalar, 2, 1> measurementXYPosition(
    const FilterState &state) {
  return state.position.block(0, 0, 2, 1);
}

Eigen::Matrix<TranslationType::scalar, 2, 1> measurementDelayedXYPosition(
    const Eigen::Vector2d &delayed_position) {
  return delayed_position;
}

template <typename FilterState>
Eigen::Matrix<TranslationType::scalar, 1, 1> measurementZPosition(
    const FilterState &state) {
  return state.position.block(2, 0, 1, 1);
}

template <typename FilterState>
Eigen::Matrix<TranslationType::scalar, 1, 1> measurementPressureSensor(
    const FilterState &state, const Eigen::Vector3d &pressure_sensor_in_imu,
    double atmospheric_pressure) {
  Eigen::Vector3d pressure_sensor_in_nav =
      state.position + state.orientation * pressure_sensor_in_imu;
  Eigen::Matrix<TranslationType::scalar, 1, 1> pressure;
  pressure << atmospheric_pressure - pressure_sensor_in_nav.z() *
                                         state.gravity(0) *
                                         state.water_density(0);
  return pressure;
}

template <typename FilterState>
VelocityType measurementVelocity(const FilterState &state) {
  // return expected velocities in the IMU frame
  return VelocityType(state.orientation.inverse() * state.velocity);
}

template <typename FilterState>
AccelerationType measurementAcceleration(const FilterState &state) {
  // returns expected accelerations in the IMU frame
  return AccelerationType(state.orientation.inverse() *
                              (Eigen::Vector3d(state.acceleration) +
                               Eigen::Vector3d(0., 0., state.gravity(0))) +
                          Eigen::Vector3d(state.bias_acc));
}
 ...
 */
MTK_BUILD_MANIFOLD(
    SimplePoseStateWithMarker,
    ((ukfom::mtkwrap<SimplePoseState>, filter_state))(
        (TranslationType,
         marker_position))  // position of a marker in navigation frame
    ((RotationType,
      marker_orientation))  // orientation of a marker in navigation frame
)
typedef ukfom::mtkwrap<SimplePoseStateWithMarker> WSimplePoseStateWithMarker;
typedef Eigen::Matrix<SimplePoseStateWithMarker::scalar,
                      SimplePoseStateWithMarker::DOF,
                      SimplePoseStateWithMarker::DOF>
    SimplePoseStateWithMarkerCov;
typedef ukfom::mtkwrap<MTK::S2<double>> WS2Type;

template <typename FilterState>
WS2Type measurementVisualLandmark(const FilterState &state,
                                  const Eigen::Vector3d &feature_pos,
                                  const Eigen::Affine3d &cam_in_imu) {
  ...
  Eigen::Vector3d feature_in_cam =
      nav_in_cam *
      (state.marker_orientation * feature_pos + state.marker_position);
 ...
 */
MTK_BUILD_MANIFOLD(
    SimplePoseStateWithDelayedPosition,
    ((ukfom::mtkwrap<SimplePoseState>, filter_state))(
        (Translation2DType,
         delayed_position))  // delayed position in navigation frame
)
typedef ukfom::mtkwrap<SimplePoseStateWithDelayedPosition>
    WSimplePoseStateWithDelayedPosition;
typedef Eigen::Matrix<SimplePoseStateWithDelayedPosition::scalar,
                      SimplePoseStateWithDelayedPosition::DOF,
                      SimplePoseStateWithDelayedPosition::DOF>
    SimplePoseStateWithDelayedPositionCov;

template <typename FilterState>
Translation2DType measurementDelayedPosition(const FilterState &state) {
...
SimplePoseUKF::SimplePoseUKF(const Eigen::Vector3d &imu_in_nwu_pos,
  ...
  initial_state.bias_gyro = BiasType(imu_in_body.rotation() *
                                     filter_config.rotation_rate.bias_offset);
  initial_state.bias_acc =
      BiasType(imu_in_body.rotation() * filter_config.acceleration.bias_offset);
  Eigen::Matrix<double, 1, 1> gravity;
  gravity(0) = pose_estimation::GravitationalModel::WGS_84(
      filter_config.location.latitude, filter_config.location.altitude);
  ...
  gravity_var << pow(
      0.05, 2.);  // give the gravity model a sigma of 5 cm/s^2 at the start
  ...
  projection.reset(new pose_estimation::GeographicProjection(
      filter_config.location.latitude, filter_config.location.longitude));

  filter_parameter.imu_in_body = imu_in_body.translation();
  filter_parameter.acc_bias_tau = filter_config.acceleration.bias_tau;
  filter_parameter.acc_bias_offset =
      imu_in_body.rotation() * filter_config.acceleration.bias_offset;
  filter_parameter.gyro_bias_tau = filter_config.rotation_rate.bias_tau;
  filter_parameter.gyro_bias_offset =
      imu_in_body.rotation() * filter_config.rotation_rate.bias_offset;
  filter_parameter.atmospheric_pressure =
      filter_config.hydrostatics.atmospheric_pressure;
  filter_parameter.water_density_tau =
      filter_config.hydrostatics.water_density_tau;
}

SimplePoseUKF::SimplePoseUKF(const State &initial_state,
                             const Covariance &state_cov,
                             const LocationConfiguration &location,
                             const PoseUKFParameter &filter_parameter)
    : filter_parameter(filter_parameter), filter_ts(0) {
...
void SimplePoseUKF::setProcessNoiseFromConfig(
  ...
  gravity_noise
      << 1.e-12;  // add a tiny bit of noise only for numeric stability
  MTK::subblock(process_noise_cov, &State::gravity) = gravity_noise;
  Eigen::Matrix<double, 1, 1> water_density_noise;
  water_density_noise
      << (2. / (filter_config.hydrostatics.water_density_tau * imu_delta_t)) *
             pow(filter_config.hydrostatics.water_density_limits, 2.);
...
// {
//     delayed_states.reset(new
//     pose_estimation::DelayedStates<Translation2DType>(std::abs(maximum_delay)));
// }
...
void SimplePoseUKF::predictionStepImpl(double delta_t) {
  ...
  ukf->predict(boost::bind(processModel<WState>, _1, rotation_rate, projection,
                           water_density_offset, filter_parameter, delta_t),
  ...
  // {
  //     filter_ts +=
  //     pose_estimation::DelayedStates<Translation2DType>::fromSeconds(delta_t);
  //     auto sigma = ukf->sigma(); // has to be copied as MTK::subblock doesnt
  //     accept const delayed_states->pushState(filter_ts,
  //     ukf->mu().delayed_position, MTK::subblock(sigma,
...
void SimplePoseUKF::integrateDelayedPositionMeasurement(
    const XY_Position &xy_position, const Eigen::Vector2d &delayed_position) {
  checkMeasurment(xy_position.mu, xy_position.cov);
  ukf->update(xy_position.mu,
              boost::bind(measurementDelayedXYPosition, delayed_position),
              boost::bind(ukfom::id<XY_Position::Cov>, xy_position.cov),
              ukfom::accept_any_mahalanobis_distance<State::scalar>);
}

void SimplePoseUKF::integrateDelayedPositionMeasurementWithStateAugmentation(
    const XY_Position &xy_position, const Eigen::Vector2d &delayed_position,
    const Eigen::Matrix2d &cov_delayed_position) {
  checkMeasurment(xy_position.mu, xy_position.cov);
  ...
  SimplePoseStateWithDelayedPositionCov augmented_state_cov =
      SimplePoseStateWithDelayedPositionCov::Zero();
  ...
  ukfom::ukf<WSimplePoseStateWithDelayedPosition> augmented_ukf(
      augmented_state, augmented_state_cov);
  augmented_ukf.update(
      xy_position.mu,
      boost::bind(
          measurementDelayedPosition<WSimplePoseStateWithDelayedPosition>, _1),
      boost::bind(ukfom::id<XY_Position::Cov>, xy_position.cov),
      ukfom::accept_any_mahalanobis_distance<State::scalar>);
  ukf.reset(
      new MTK_UKF(augmented_ukf.mu().filter_state,
                  augmented_ukf.sigma().block(0, 0, WState::DOF, WState::DOF)));
}

void SimplePoseUKF::integrateMeasurement(
    const Pressure &pressure, const Eigen::Vector3d &pressure_sensor_in_imu) {
  checkMeasurment(pressure.mu, pressure.cov);
  ukf->update(
      pressure.mu,
      boost::bind(measurementPressureSensor<State>, _1, pressure_sensor_in_imu,
                  filter_parameter.atmospheric_pressure),
      boost::bind(ukfom::id<Pressure::Cov>, pressure.cov),
      ukfom::accept_any_mahalanobis_distance<State::scalar>);
}

void SimplePoseUKF::integrateMeasurement(const GeographicPosition &geo_position,
                                         const Eigen::Vector3d &gps_in_body) {
  ...
  projection->worldToNav(geo_position.mu.x(), geo_position.mu.y(),
                         projected_position.x(), projected_position.y());
  projected_position =
      projected_position - (ukf->mu().orientation * gps_in_body).head<2>();
...
void SimplePoseUKF::integrateMeasurement(
    const std::vector<VisualFeatureMeasurement> &marker_corners,
    const std::vector<Eigen::Vector3d> &feature_positions,
    const Eigen::Affine3d &marker_pose,
    const Eigen::Matrix<double, 6, 6> cov_marker_pose,
    const CameraConfiguration &camera_config,
    const Eigen::Affine3d &camera_in_IMU) {
  // Augment the filter state with the marker pose
  WSimplePoseStateWithMarker augmented_state;
  augmented_state.filter_state = ukf->mu();
  augmented_state.marker_position = TranslationType(marker_pose.translation());
  augmented_state.marker_orientation =
      RotationType(MTK::SO3<double>(marker_pose.rotation()));
  SimplePoseStateWithMarkerCov augmented_state_cov =
      SimplePoseStateWithMarkerCov::Zero();
  augmented_state_cov.block(0, 0, WState::DOF, WState::DOF) = ukf->sigma();
  augmented_state_cov.bottomRightCorner<6, 6>() = cov_marker_pose;
  ukfom::ukf<WSimplePoseStateWithMarker> augmented_ukf(augmented_state,
                                                       augmented_state_cov);
  ...
  for (unsigned i = 0;
       i < marker_corners.size() || i < feature_positions.size(); i++) {
    checkMeasurment(marker_corners[i].mu, marker_corners[i].cov);

    // project image coordinates into S2
    WS2Type projection(MTK::S2<double>(
        (marker_corners[i].mu.x() - camera_config.cx) / camera_config.fx,
        (marker_corners[i].mu.y() - camera_config.cy) / camera_config.fy, 1.0));
    std::cout << "Corner: " << i << " , Projection: " << projection
              << std::endl;
    Eigen::Matrix2d projection_cov;
    projection_cov << marker_corners[i].cov(0, 0) / fx2,
        marker_corners[i].cov(0, 1) / fxy, marker_corners[i].cov(1, 0) / fxy,
        marker_corners[i].cov(1, 1) / fy2;

    augmented_ukf.update(
        projection,
        boost::bind(measurementVisualLandmark<WSimplePoseStateWithMarker>, _1,
                    feature_positions[i], camera_in_IMU),
        boost::bind(ukfom::id<VisualFeatureMeasurement::Cov>, projection_cov),
        ukfom::accept_any_mahalanobis_distance<
            WSimplePoseStateWithMarker::scalar>);
  }

  // Reconstructing the filter is currently the only way to modify the internal
  // state of the filter
...
// bool PoseUKF::integrateDelayedMeasurement(const XY_Position &xy_position,
// double delay)
// {
//     checkMeasurment(xy_position.mu, xy_position.cov);
//     int64_t measurement_ts = filter_ts -
//     pose_estimation::DelayedStates<Translation2DType>::fromSeconds(delay);
//     Translation2DType delayed_position;
//     pose_estimation::DelayedStates<Translation2DType>::Cov
//     cov_delayed_position; if (delayed_states &&
//     delayed_states->getClosestState(measurement_ts, delayed_position,
//     cov_delayed_position))
//     {
//         // update current state with closest delayed state
//         Translation2DType current_delayed_position =
//         ukf->mu().delayed_position; WState delayed_state = ukf->mu();
//         delayed_state.delayed_position = delayed_position;
//         ukf.reset(new MTK_UKF(delayed_state, ukf->sigma()));

//         // integrate delayed measurement, currently ignoring the difference
//         in uncertainty at the delayed state ukf->update(xy_position.mu,
//         boost::bind(measurementDelayedXYPosition<WState>, _1),
//                     boost::bind(ukfom::id<XY_Position::Cov>,
//                     xy_position.cov), d2p95<State::scalar>);
...
void SimplePoseUKF::resetFilterWithExternalPose(
...
SimplePoseUKF::RotationRate::Mu SimplePoseUKF::getRotationRate() {
  double latitude, longitude;
  projection->navToWorld(ukf->mu().position.x(), ukf->mu().position.y(),
                         latitude, longitude);
  Eigen::Vector3d earth_rotation =
      Eigen::Vector3d(pose_estimation::EARTHW * cos(latitude), 0.,
                      pose_estimation::EARTHW * sin(latitude));
  return rotation_rate - ukf->mu().bias_gyro -
