#ifndef _UWV_KALMAN_FILTERS_SIMPLE_POSE_UKF_HPP_
#define _UWV_KALMAN_FILTERS_SIMPLE_POSE_UKF_HPP_
 ...
 */
class SimplePoseUKF
    : public pose_estimation::UnscentedKalmanFilter<SimplePoseState> {
 public:
  /**
 ...
 public:
   ...
   */
  SimplePoseUKF(const State &initial_state, const Covariance &state_cov,
                const LocationConfiguration &location,
                const PoseUKFParameter &filter_parameter);

  virtual ~SimplePoseUKF() {}
   ...
   * (optional)
   */
  void setProcessNoiseFromConfig(
      const PoseUKFConfig &pose_filter_config, double imu_delta_t,
  ...
  void integrateMeasurement(
      const GeographicPosition &geo_position,
  ...
  /* 2D delayed position expressed in the NWU-navigation frame, also needs
   * delayed position state for measurement function*/
  void integrateDelayedPositionMeasurement(
      const XY_Position &xy_position, const Eigen::Vector2d &delayed_position);
  void integrateDelayedPositionMeasurementWithStateAugmentation(
      const XY_Position &xy_position, const Eigen::Vector2d &delayed_position,
  ...
  void integrateMeasurement(const BodyEffortsMeasurement &body_efforts,
                            bool only_affect_velocity = false);

  /* Water Velocities from ADCP expressed in the IMU frame */
  void integrateMeasurement(const WaterVelocityMeasurement &adcp_measurements,
                            double cell_weighting);

  /**
   * The features (usually the four corners) of a visual marker in undistorted
   * image coordinates. |marker_features| and |feature_positions| musst be of
   * equal size and order.
   * @param marker_features image coordinates of the features in the undistorted
   * image.
   * @param feature_positions are the positions of the featues in the marker
   * frame.
   */
  void integrateMeasurement(
      const std::vector<VisualFeatureMeasurement> &marker_features,
